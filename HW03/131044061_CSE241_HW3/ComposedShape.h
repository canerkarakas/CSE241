...
public:
	...
	public:
		ShapeElem();
		ShapeElem(void *p, int);
		const double area();
		const double length();
		void draw(ofstream &);
		void* get_shape() const;
		int get_smallShape() const;
		friend ofstream& operator <<(ofstream &, const ShapeElem &);
	private:
		void *shape;
	...
	const ComposedShape& operator+=(ComposedShape::ShapeElem &);
	const ShapeElem operator[](int);
	friend ofstream& operator<<(ofstream &, const ComposedShape &);
	ShapeElem* get_smallShapes() const;
