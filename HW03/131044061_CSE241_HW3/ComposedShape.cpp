...
void ComposedShape::optimalFit(Rectangle& contRect, Rectangle& smallRect, ofstream &ofs) {
	...
	int contShapeArea = contRect.get_width() * contRect.get_height() ;
	int smallShapeArea = smallRect.get_width() * smallRect.get_height();
	//int countForShape = 0;
	...
	while(countHeight>0){
		while(countWidth>0){
			...
			if(((crd_x[x] + smallRect.get_width()) < contRect.get_width()) ||
				...
				smallRect.set_coords(crd_x[x], crd_y[y]);
				ComposedShape::ShapeElem smallShape(&smallRect, 0);
				smallShape.draw(ofs);
				smallShapes.push_back(smallShape);
				//smallRect.draw(ofs);
			...
			if(countForFree>smallHeight || countForFree==smallHeight){
				...
				while(control>0){
					if((crd_x[x] + tempWidth) < contRect.get_width()
						...
						ComposedShape::ShapeElem tempSmall(&temp, 0);
						tempSmall.draw(ofs);
						smallShapes.push_back(tempSmall);
					//	temp.draw(ofs);
	...
	int diffArea = contShapeArea - ((secondCountForShape+firstCountForShape)*smallShapeArea);
	...
	/*cout<<smallRect.result_area(smallRect)+temp.result_area(temp)<<endl;
	cout<<countForShape*smallShapeArea;*/
...
void ComposedShape::optimalFit(Rectangle& contRect, Circle& smallCirc, ofstream &ofs) {
	...
	while(control1>0){
		...
		ComposedShape::ShapeElem smallShape(&smallCirc, 1);
		smallShape.draw(ofs);
		smallShapes.push_back(smallShape);
		//smallCirc.draw(ofs);
		...
		while(control2 == 1){
			...
			if((crd_x[x]+jumpControl) > contRect.get_width()){
				...
				x++;
				smallCirc.set_coords(crd_x[x], crd_y[y]);
				ComposedShape::ShapeElem tempShape(&smallCirc, 1);
				tempShape.draw(ofs);
				smallShapes.push_back(tempShape);
			//	smallCirc.draw(ofs);
				countForShape = smallCirc.get_numOfShape();
				//countForShape = countForShape + 1;
			}
		...
		if(points[2][1]>contRect.get_height() || points[0][1]>contRect.get_height() ||
			...
			//control points for overflow
				if(points[2][0]>contRect.get_width() || points[2][1]>contRect.get_height() ||
					...
					ComposedShape::ShapeElem smallShape(&smallTri, 2);
					smallShape.draw(ofs);
					//smallTri.draw(ofs);
					smallShapes.push_back(smallShape);
...
ofstream& operator <<(ofstream& ofs, const ComposedShape& comp) {
...
ComposedShape::ShapeElem::ShapeElem(void *smallShape, int shape) {
	ComposedShape::ShapeElem::smallShape = static_cast<ComposedShape::ShapeElem::Shapes>(shape);
	ComposedShape::ShapeElem::shape = smallShape;
}

const double ComposedShape::ShapeElem::area() {
	double result;
	if(this->get_smallShape() == rectangle){
		Rectangle temp;
		temp = *static_cast<Rectangle*>(this->get_shape());
		result = temp.area();
	}
	if(this->get_smallShape() == circle){
		Circle temp;
		temp = *static_cast<Circle*>(this->get_shape());
		result = temp.area();
	}
	if(this->get_smallShape() == triangle){
		Triangle temp;
		temp = *static_cast<Triangle*>(this->get_shape());
		result = temp.area();
	}
	return result;
}

const double ComposedShape::ShapeElem::length() {
	double result;
	if(this->get_smallShape() == rectangle){
		Rectangle temp;
		temp = *static_cast<Rectangle*>(this->get_shape());
...
void ComposedShape::ShapeElem::draw(ofstream& ofs) {
	if(this->get_smallShape() == rectangle){
		Rectangle temp;
		temp = *static_cast<Rectangle*>(this->get_shape());
		ofs<<*this;
...
ofstream& operator <<(ofstream& ofs, const ComposedShape::ShapeElem& shape) {
	if(shape.get_smallShape() == ComposedShape::ShapeElem::rectangle){
		Rectangle temp;
		temp = *static_cast<Rectangle*>(shape.get_shape());
		temp.draw(ofs);
	}
	if(shape.get_smallShape() == ComposedShape::ShapeElem::circle){
		Circle temp;
		temp = *static_cast<Circle*>(shape.get_shape());
		temp.draw(ofs);
	}
	if(shape.get_smallShape() == ComposedShape::ShapeElem::triangle){
		Triangle temp;
		temp = *static_cast<Triangle*>(shape.get_shape());
		temp.draw(ofs);
	}
	return ofs;
}

const ComposedShape& ComposedShape::operator +=(ComposedShape::ShapeElem &shape) {
	this->smallShapes.push_back(shape);
	return *this;
}

const ComposedShape::ShapeElem ComposedShape::operator [](int index) {
	return this->get_smallShapes()[index];
}

ComposedShape::ShapeElem* ComposedShape::get_smallShapes() const {
	return this->get_smallShapes();
}
...
int ComposedShape::ShapeElem::get_smallShape() const{
	return this->smallShape;
}
